...
class StoreProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'variants.*.opening_stock_date' => ['nullable', 'date'],
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator) {
            if ($this->input('type') === 'variant') {
                $variationId = (int) $this->input('variation_id');
                $variants = $this->input('variants', []);
                if (is_array($variants) && $variationId > 0) {
            ...
            if ($this->input('type') === 'single' && $barcode !== '') {
                $taken = Product::query()->where('barcode', $barcode)->exists()
            ...
            foreach (array_values($variants) as $index => $variant) {
                ...
                $barcodeTaken = ProductVariant::query()->where('barcode', $vBarcode)->exists()
                    || Product::query()->where('barcode', $vBarcode)->exists();
                if ($barcodeTaken) {
    ...
    public function payload(): array
    {
        ...
        if ($type === 'single') {
            $product['barcode'] = $this->input('barcode') ?: null;
            $product['purchase_unit_id'] = $this->input('purchase_unit_id');
            $product['sale_unit_id'] = $this->input('sale_unit_id');
            $product['conversion_rate'] = $this->input('conversion_rate');
            $product['sale_price'] = $this->input('sale_price');
            $product['cost_per_unit'] = $this->input('cost_per_unit', 0) ?: 0;
            $product['opening_stock'] = $this->input('opening_stock');
            $product['opening_stock_date'] = $this->input('opening_stock_date');
            $product['section_id'] = $this->input('section_id') ?: null;
            $product['rack_id'] = $this->input('rack_id') ?: null;
            ...
            $first = $variants[0];
            $product['barcode'] = ($first['barcode'] ?? null) ?: null;
            $product['purchase_unit_id'] = $first['purchase_unit_id'];
            $product['sale_unit_id'] = $first['sale_unit_id'];
            $product['conversion_rate'] = $first['conversion_rate'];
            $product['sale_price'] = $first['sale_price'];
            $product['cost_per_unit'] = $first['cost_per_unit'] ?? 0;
        }
