...
export default function SectionImportDrawer({ open, onClose, result = null }) {
    ...
    const submit = (e) => {
        ...
        form.post(route('admin.import-export.sections.import'), {
            forceFormData: true,
            preserveScroll: true,
    ...
    return (
        <Drawer
            ...
            description="Upload CSV or Excel. Columns: name, code, rack_name, rack_code, is_active."
            width="half"
        >
            <div className="flex h-full flex-col gap-5">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-xs text-theme-ink-muted">
                        One row per rack. Same section name/code groups into one section.
                    </p>
                    <div className="flex flex-wrap gap-2">
                        <a
                            href={route('admin.import-export.sections.sample', { format: 'xlsx' })}
                            className="rounded-lg border border-theme-border px-3 py-2 text-sm font-medium text-theme-ink hover:bg-theme-bg"
                        >
                            Excel sample
                        </a>
                        <a
                            href={route('admin.import-export.sections.sample', { format: 'csv' })}
                            className="rounded-lg border border-theme-border px-3 py-2 text-sm font-medium text-theme-ink hover:bg-theme-bg"
                        >
                            CSV sample
                        </a>
                ...
                <form onSubmit={submit} className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2 border-t border-theme-border pt-4">
                        ...
                        <Button type="submit" disabled={form.processing || !form.data.file}>
                            {form.processing ? 'Importing…' : 'Import sections'}
                        </Button>
                    </div>
                </form>
            </div>
