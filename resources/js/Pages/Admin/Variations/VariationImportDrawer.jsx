...
export default function VariationImportDrawer({ open, onClose, result = null }) {
    ...
    const submit = (e) => {
        ...
        form.post(route('admin.import-export.variations.import'), {
            forceFormData: true,
            preserveScroll: true,
    ...
    return (
        <Drawer
            ...
            description="Upload CSV or Excel. Columns: name, code, option_name, option_code, is_active."
            width="half"
        >
            <div className="flex h-full flex-col gap-5">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-xs text-theme-ink-muted">
                        One row per option. Same name/code groups into one variation type.
                    </p>
                    <div className="flex flex-wrap gap-2">
                        <a
                            href={route('admin.import-export.variations.sample', { format: 'xlsx' })}
                            className="rounded-lg border border-theme-border px-3 py-2 text-sm font-medium text-theme-ink hover:bg-theme-bg"
                        >
                            Excel sample
                        </a>
                        <a
                            href={route('admin.import-export.variations.sample', { format: 'csv' })}
                            className="rounded-lg border border-theme-border px-3 py-2 text-sm font-medium text-theme-ink hover:bg-theme-bg"
                        >
                            CSV sample
                        </a>
                ...
                <form onSubmit={submit} className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2 border-t border-theme-border pt-4">
                        ...
                        <Button type="submit" disabled={form.processing || !form.data.file}>
                            {form.processing ? 'Importing…' : 'Import variations'}
                        </Button>
                    </div>
                </form>
            </div>
