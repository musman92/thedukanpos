...
export default function Index({ suppliers, filters }) {
    ...
    const destroySupplier = async (supplier) => {
        const ok = await confirmDelete(supplier.name, 'supplier');
        if (!ok) return;
    ...
    return (
        <AdminLayout
            title="Suppliers"
            description="Vendors and distributors you purchase from."
            actions={
        ...
        >
                    ...
                    <table className="min-w-full table-fixed text-left text-sm">
                        ...
                        <tbody>
                            ...
                            {suppliers.data.map((supplier, idx) => (
                                <tr key={supplier.id} className="border-t border-theme-border">
                                    <td className="px-3 py-3 text-theme-ink-muted">
                                        {(suppliers.from || 1) + idx}
                                    </td>
                                    <td className="px-3 py-3 text-theme-ink-soft">{supplier.code || '—'}</td>
                                    <td className="px-3 py-3 font-medium text-theme-ink">
                                        <div>{supplier.name}</div>
                                        {supplier.contact_person && (
                                            <div className="text-xs font-normal text-theme-ink-muted">
                                                {supplier.contact_person}
                                            </div>
                                        )}
                                    </td>
                                    ...
                                    <td className="px-3 py-3 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            ...
                                            <button
                                                ...
                                                title="Delete"
                                                aria-label="Delete"
                                                onClick={() => destroySupplier(supplier)}
            ...
            <SupplierFormDrawer open={showForm} supplier={editing} onClose={closeForm} />
        </AdminLayout>
    );
}
