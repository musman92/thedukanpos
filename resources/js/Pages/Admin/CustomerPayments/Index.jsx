...
export default function Index({
    ...
    form_customer_id: formCustomerId = null,
    ...
    }, [formOpen, formCustomerId, editingPayment?.id]);
    ...
    const openCreate = () => {
        setShowForm(true);
        visitList({ open: 1, form_customer_id: undefined, form_payment_id: undefined });
    };
    ...
    const openEdit = (payment) => {
        ...
        visitList({
            open: 1,
            form_customer_id: payment.customer_id,
    ...
    const closeForm = () => {
        setShowForm(false);
        visitList({ open: undefined, form_customer_id: undefined, form_payment_id: undefined });
    };

    const destroyPayment = async (payment) => {
        const label = `${payment.customer?.name || 'Payment'} · ${money(payment.amount)}`;
        const ok = await confirmDelete(label, 'customer payment');
        if (!ok) return;

        router.delete(route('admin.finance.customer-payments.destroy', payment.id), {
            preserveScroll: true,
        });
    };

    const title = branch?.name
        ? `Customer payments · ${branch.name}`
        : 'Customer payments';

    return (
        <AdminLayout
            title={title}
            description="Collect unpaid sales, opening balance, or record an advance."
            actions={
            ...
            <div className="dp-card overflow-hidden">
                <div className="space-y-3 border-b border-theme-border px-4 py-3">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <PageLimitSelect
                            pageKey="customer-payments"
                            routeName="admin.finance.customer-payments.index"
                        ...
                        <thead className="bg-theme-bg text-[11px] uppercase tracking-wide text-theme-ink-muted">
                            <tr>
                                ...
                                <SortableTh
                                    ...
                                    onSort={toggleSort}
                                />
                                <th className="px-3 py-3 font-semibold">Received in</th>
                                ...
                                <tr key={payment.id} className="border-t border-theme-border">
                                    ...
                                    <td className="px-3 py-3 text-right tabular-nums font-medium text-theme-ink">
                                        ...
                                        {Number(payment.discount_amount || 0) > 0 && (
                                            <span className="mt-0.5 block text-xs font-normal text-theme-ink-muted">
                                                +{money(payment.discount_amount)} write-off
                                            </span>
                                        )}
                                    </td>
                                    ...
                                    <td className="max-w-[14rem] truncate px-3 py-3 text-xs text-theme-ink-muted">
                                        {(payment.sales || []).length > 0
                                            ? payment.sales.map((p) => p.number).join(', ')
