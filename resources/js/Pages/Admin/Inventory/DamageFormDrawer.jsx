...
function dataFromDamage(damage, variants = []) {
    if (!damage) return emptyData();

    return {
        reason: damage.reason || '',
        notes: damage.notes || '',
        items: (damage.items || []).map((item) => {
            const catalog = variants.find((v) => String(v.id) === String(item.variant_id));
            const currentOnHand = Number(catalog?.quantity_on_hand || 0);
            const damagedQty = Number(item.quantity || 0);

            return {
                variant_id: String(item.variant_id),
                quantity: String(item.quantity ?? ''),
                display_name: item.display_name || item.variant?.name || catalog?.label || '—',
                short_code: item.short_code || item.variant?.short_code || catalog?.short_code || '',
                sale_unit_name: item.unit_name || catalog?.sale_unit_name || 'pcs',
                // Stock already reduced by this damage — restore for available check.
                quantity_on_hand: currentOnHand + damagedQty,
                average_cost: Number(item.unit_cost ?? catalog?.average_cost ?? 0),
            };
        }),
    };
}

export default function DamageFormDrawer({
    open,
    damage = null,
    variants = [],
    reasons = [],
    ...
    useEffect(() => {
        ...
        form.clearErrors();
        setSubmitError('');
        form.setData(dataFromDamage(damage, variants));
        setPickerKey((k) => k + 1);
    ...
    const addFromCatalog = (variantId) => {
        ...
        if (selectedIds.has(String(variant.id))) {
            setSubmitError('This product is already listed.');
            setPickerKey((k) => k + 1);
            return;
        }
        ...
        form.setData('items', [
            ...form.data.items,
            {
                ...
                average_cost: Number(variant.average_cost || 0),
            },
        ]);
        setSubmitError('');
        setPickerKey((k) => k + 1);
    ...
    const submit = (e) => {
        ...
        if (!form.data.reason) {
            setSubmitError('Select a damage reason.');
            return;
        }

        if (form.data.reason === 'other' && !String(form.data.notes || '').trim()) {
            setSubmitError('Notes are required when reason is Other.');
            return;
        }
        ...
        if (editing) {
            form.post(route('admin.inventory.damages.update', damage.id), options);
        } else {
            form.post(route('admin.inventory.damages.store'), options);
        }
    };
    ...
    return (
        <Drawer
            open={open}
            onClose={onClose}
            title={editing ? 'Edit damage' : 'Record damage'}
            description="Write off stock lost to expiry, breakage, leakage, or product fault. Quantity can only decrease."
            width="wide"
            ...
            <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Field label="Reason" required error={form.errors.reason}>
                            <SearchableSelect
                                ...
                                onChange={(value) => {
                                    form.setData('reason', value ? String(value) : '');
                                    setSubmitError('');
                                }}
                            ...
                            <table className="min-w-full text-left text-sm">
                                <thead className="bg-theme-bg text-[11px] uppercase tracking-wide text-theme-ink-muted">
                                    <tr>
                                        ...
                                        <th className="w-44 px-3 py-3 font-semibold">
                                            Damage qty
                                        </th>
                    ...
                    <Field
                        ...
                        hint={
                            form.data.reason === 'other'
                                ? 'Required for Other — describe the loss.'
                                : 'Optional details (batch, invoice, etc.).'
                        }
                    >
                        <TextArea
                            rows={3}
                            ...
                            placeholder="Details about this loss"
                        />
                    </Field>
                ...
                <div className="mt-auto flex justify-end gap-2 border-t border-theme-border pt-5">
                    ...
                    <Button type="submit" disabled={form.processing}>
                        {editing ? 'Update damage' : 'Save damage'}
                    </Button>
                </div>
            </form>
        </Drawer>
